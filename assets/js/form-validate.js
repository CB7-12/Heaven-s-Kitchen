	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#heavenskitchen-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#heavenskitchen-contact').find('input,textarea').attr({value:''});
				jQuery('#heavenskitchen-contact').css({opacity:1});
				jQuery('#heavenskitchen-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#heavenskitchen-contact').animate({opacity:0.3});
		jQuery('#heavenskitchen-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#heavenskitchen-contact').find('input,textarea').css('border','none');
